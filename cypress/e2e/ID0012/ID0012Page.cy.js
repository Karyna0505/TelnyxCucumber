...
class SwithSave{
    ...
    static switchSlider(){
        cy.get('div:nth-child(4) > div> div > div.ant-slider-handle').type(arrows);
        cy.get('div:nth-child(5) > div.telnyx-slider > div > div.ant-slider-handle').type(arrows);
        
        
        
    }
    static checkChangeCost(){
        cy.get(costTelnyx).should(($div) => {
           
            const text = $div.text();
            expect(text).not.to.include('$2,578')})

        cy.get(costTwilio).should(($div) => {
